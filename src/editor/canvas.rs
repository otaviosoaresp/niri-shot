...
impl EditorCanvas {
    ...
    fn draw(&self, ctx: &cairo::Context, _width: i32, _height: i32) {
        ...
        for (idx, shape) in imp.shapes.borrow().iter().enumerate() {
            ...
            if is_selected {
                self.draw_selection_handles(ctx, shape, if is_moving { (offset_x, offset_y) } else { (0.0, 0.0) });
            }
        }
    ...
    fn draw_selection_handles(&self, ctx: &cairo::Context, shape: &Shape, offset: (f64, f64)) {
        ...
        ctx.rectangle(min_x - 3.0, min_y - 3.0, max_x - min_x + 6.0, max_y - min_y + 6.0);
        let _ = ctx.stroke();
        ctx.set_dash(&[], 0.0);
        ...
        for (hx, hy) in [
            ...
            ctx.rectangle(hx - handle_size / 2.0, hy - handle_size / 2.0, handle_size, handle_size);
            let _ = ctx.fill();
        }
